...
for sampleSize in [1200,2000,2400,2800,3600,4000,4400,4800,5200,5600,6000,12800,25600,51200]:
	sim_param = {'N': sampleSize, 'sigma': 0.5, 'train_ratio': 0.75}
	for alpha in [0.5]:
		n_repeat = 1
		...
		for iteration in range(n_repeat): # number of iteration = 2
			for md in [0,1,2,3,4,5,6]:
				...
				end_time = time.time()
				times_train['TT_mean_Depth_'+str(md)].append(end_time - start_time)
				times_train['TT_CP_Depth_'+str(md)].append(end_time - start_time)
				times_train['TT_Tucker_Depth_'+str(md)].append(end_time - start_time)
				...
				end_time = time.time()
				times_test['TT_mean_Depth_'+str(md)].append(end_time - start_time + times_train['TT_mean_Depth_'+str(md)][-1])
				
				train_MSE, test_MSE = np.mean((TT_mean_train_pred - data.train_y)**2), np.mean((TT_mean_pred - data.test_y)**2)
				results_train['TT_mean_Depth_'+str(md)].append(train_MSE)
				results_test['TT_mean_Depth_'+str(md)].append(test_MSE)
				...
				end_time = time.time()
				times_test['TT_CP_Depth_'+str(md)].append(end_time - start_time + times_train['TT_CP_Depth_'+str(md)][-1])
				...
				results_train['TT_CP_Depth_'+str(md)].append(train_MSE)
				results_test['TT_CP_Depth_'+str(md)].append(test_MSE)
				...
				end_time = time.time()
				times_test['TT_Tucker_Depth_'+str(md)].append(end_time - start_time + times_train['TT_Tucker_Depth_'+str(md)][-1])
				...
				results_train['TT_Tucker_Depth_'+str(md)].append(train_MSE)
				results_test['TT_Tucker_Depth_'+str(md)].append(test_MSE)
				
				
				# ------------------------ #
				# - Fit-Prune tensorTree mean  - #
				# ------------------------ #
				print("Fit-Prune tensorTree mean...")
				TT_tensor_regression_model = TensorDecisionTreeRegressor(max_depth=md, min_samples_split=2, split_method='variance',split_rank=3,n_mode=4) # split_rank=3 to match the TensorGPST empirically works well
				TT_tensor_regression_model.sample_rate = 0.1
				start_time = time.time()
				TT_tensor_regression_model.fit(data.train_X, data.train_y)
				...
				end_time = time.time()
				times_train['TT_mean_prune_Depth_'+str(md)].append(end_time - start_time)
				times_train['TT_CP_prune_Depth_'+str(md)].append(end_time - start_time)
				times_train['TT_Tucker_prune_Depth_'+str(md)].append(end_time - start_time)
				...
				TT_mean_pred = TT_tensor_regression_model.predict(data.test_X,regression_method='mean')
				end_time = time.time()
				times_test['TT_mean_prune_Depth_'+str(md)].append(end_time - start_time + times_train['TT_mean_prune_Depth_'+str(md)][-1])
				
				train_MSE, test_MSE = np.mean((TT_mean_train_pred - data.train_y)**2), np.mean((TT_mean_pred - data.test_y)**2)
				results_train['TT_mean_prune_Depth_'+str(md)].append(train_MSE)
				results_test['TT_mean_prune_Depth_'+str(md)].append(test_MSE)
				
				# ------------------------ #
				# -   Fit-Prune tensorTree CP  - #
				# ------------------------ #
				print("Fit-Prune tensorTree CP...")
				start_time = time.time()
				TT_tensor_regression_model.fit(data.train_X, data.train_y)
				TT_tensor_regression_model.prune(data.train_X,data.train_y,'cp',alpha)
				TT_CP_train_pred = TT_tensor_regression_model.predict(data.train_X,regression_method='cp')
				end_time = time.time()
				times_test['TT_CP_prune_Depth_'+str(md)].append(end_time - start_time + times_train['TT_CP_prune_Depth_'+str(md)][-1])

				TT_CP_pred = TT_tensor_regression_model.predict(data.test_X,regression_method='cp')
				train_MSE, test_MSE = np.mean((TT_CP_train_pred - data.train_y)**2), np.mean((TT_CP_pred - data.test_y)**2)
				results_train['TT_CP_prune_Depth_'+str(md)].append(train_MSE)
				results_test['TT_CP_prune_Depth_'+str(md)].append(test_MSE)

				# ------------------------ #
				# - Fit-Prune tensorTree Tucker- #
				# ------------------------ #
				print("Fit-Prune tensorTree Tucker...")
				start_time = time.time()
				TT_tensor_regression_model.fit(data.train_X, data.train_y)
				TT_tensor_regression_model.prune(data.train_X,data.train_y,'tucker',alpha)
				TT_Tucker_train_pred = TT_tensor_regression_model.predict(data.train_X,regression_method='tucker')
				end_time = time.time()
				times_test['TT_Tucker_prune_Depth_'+str(md)].append(end_time - start_time + times_train['TT_Tucker_prune_Depth_'+str(md)][-1])

				TT_Tucker_pred = TT_tensor_regression_model.predict(data.test_X,regression_method='tucker')
				train_MSE, test_MSE = np.mean((TT_Tucker_train_pred - data.train_y)**2), np.mean((TT_Tucker_pred - data.test_y)**2)
				results_train['TT_Tucker_prune_Depth_'+str(md)].append(train_MSE)
				results_test['TT_Tucker_prune_Depth_'+str(md)].append(test_MSE)
				print('depth=',md,'--->')
		...
		# Save to CSV
		df_train.to_csv('SS_'+str(sampleSize)+'methodPrune_train_'+str(alpha)+'.csv')
		df_test.to_csv('SS_'+str(sampleSize)+'methodPrune_test_'+str(alpha)+'.csv')
		...
		# Save to CSV
		dft_train.to_csv('SS_'+str(sampleSize)+'methodPrune_train_time_'+str(alpha)+'.csv')
		dft_test.to_csv('SS_'+str(sampleSize)+'methodPrune_test_time_'+str(alpha)+'.csv')
