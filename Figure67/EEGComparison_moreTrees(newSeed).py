...
for md in [0,1,2,3,4,5,6]:
    ...
    for iteration in range(n_repeat): # number of iteration = 2
        ...
        x = np.load('../EEG/EEG_scalar.npy')
        y = np.load('../EEG/EEG_tensor.npy')
        print(x.shape,y.shape)
        my_scalar = y.squeeze()
        my_tensor = np.transpose(x, (2, 0, 1))
        print(x.shape,y.shape)
        data.train_X = x[1:31,:,:]
        data.train_y = y[1:31]
        ...
        TT_tensor_regression_model = TensorDecisionTreeRegressor(max_depth=md, min_samples_split=2, split_method='variance',split_rank=3,n_mode=3) # split_rank=3 to match the TensorGPST empirically works well
    ...
    # Save to CSV
    df_train.to_csv('EEGComparison_train_depth_'+str(md)+'1.csv')
    df_test.to_csv('EEGComparison_test_depth_'+str(md)+'1.csv')
    ...
    # Save to CSV
    dft_train.to_csv('EEGComparison_train_depth_'+str(md)+'_time1.csv')
    dft_test.to_csv('EEGComparison_test_depth_'+str(md)+'_time1.csv')
